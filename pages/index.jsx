...
const callGemini = async (prompt, retries = 3, delay = 1000) => {
    // Basic check if key is missing or still the placeholder
    ...
    try {
     ...
     // ... rest of the code
            ...
            if (!authResponse.data?.user && !authResponse.data?.session) {
                 // If email confirmation is enabled, user might be null but session might exist or vice versa
                 // For now, let's assume direct login or throw specific error if needed
                 if(authResponse.error?.message.includes("confirmation")) {
                    setError("Please check your email to confirm your account."); // Adjust message as needed
                    setLoading(false);
            ...
            // Use user from session if available, otherwise from user object
            ...
            // Step 2: Call backend API *only* to create Supabase DB record
            ...
            const backendResponse = await fetch('/api/registerUser', {
                ...
                // No Authorization header needed here if backend trusts client signup implicitly
                // OR pass back session token if backend needs verification for this step
            ...
            if (!backendResponse.ok) {
                ...
                // Attempt to sign out the potentially created user if DB fails
            ...
            // onAuthStateChange listener below will update UI after signup potentially logs in
            ...
            if (authResponse.error) throw authResponse.error;
            // onAuthStateChange listener below handles UI update
        }

    } catch (err) {
        console.error("Supabase Auth Error:", err);
        let errorMessage = err.message || 'An unknown error occurred.';
        if (errorMessage.includes("already registered") || errorMessage.includes("unique constraint")) { // Handle DB unique constraint too
...
const Header = ({ view, setIsMobileMenuOpen, userEmail }) => { /* ... updated handleLogout ... */
    const viewTitles = { home: 'Dashboard', elections: 'Active Elections', results: 'Live Results', verification: 'Verify Vote' };
    const handleLogout = async () => { const { error } = await supabase.auth.signOut(); if (error) console.error("Error signing out:", error); };
...
export default function App() {
  ...
  const [session, setSession] = useState(null); // Use Supabase session state
  ...
  // Use session?.user to check if logged in
  if (!session?.user) {
    console.log("No Supabase session, showing LoginView.");
    return <LoginView />;
  }

  // --- Logged-in App ---
  console.log("User logged in:", session.user.email, "Rendering main app...");
  const renderView = () => {
    switch (view) {
