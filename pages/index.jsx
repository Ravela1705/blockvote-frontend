...
import {
  getAuth,
  onAuthStateChanged,
  createUserWithEmailAndPassword, // Import client-side function
...
const auth = getAuth(app); // Get client-side auth instance
...
const callGemini = async (prompt, retries = 3, delay = 1000) => {
  ...
  try {
    ...
    if (!response.ok) { if ((response.status === 429 || response.status >= 500) && retries > 0) { await new Promise(resolve => setTimeout(resolve, delay)); return callGemini(prompt, retries - 1, delay * 2); } throw new Error(`API Error: ${response.statusText}`); }
    ...
    console.warn("Unexpected Gemini response:", result); return "Could not generate response.";
  } catch (error) { console.error("Error calling Gemini:", error); if (retries > 0) { await new Promise(resolve => setTimeout(resolve, delay)); return callGemini(prompt, retries - 1, delay * 2); } return `Error: ${error.message}.`; }
};
...
const LoginView = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isRegistering) {
        // Step 1: Create user client-side
        console.log("Attempting client-side registration for:", email);
        const userCredential = await createUserWithEmailAndPassword(auth, email, password);
        const user = userCredential.user;
        console.log("Client-side registration successful:", user.uid);

        // Step 2: Get ID token
        const token = await getIdToken(user, true); // Force refresh token

        // Step 3: Call backend API *only* to create Firestore record
        console.log("Calling backend to create Firestore record for:", user.uid);
        const response = await fetch('/api/registerUser', {
          ...
          // Send UID and email, password not needed here anymore
        ...
        if (!response.ok) {
          // If backend fails, try to clean up the Auth user? (Complex, skip for now)
          console.error("Backend Firestore creation failed:", data.error);
          throw new Error(data.error || 'Failed to complete registration on server.');
        }
        console.log("Backend Firestore creation successful.");
        // Login happens automatically after createUserWithEmailAndPassword
        // The onAuthStateChanged listener will handle the UI update

      } else {
        // Handle Login (No change needed here)
        await signInWithEmailAndPassword(auth, email, password);
      }
    } catch (err) {
      console.error("Auth Error:", err);
      let errorMessage = 'An unknown error occurred.';
       if (err instanceof Error && err.code) { // Check Firebase Auth error codes
           switch (err.code) {
               case 'auth/email-already-in-use': errorMessage = 'This email address is already registered.'; break;
               case 'auth/weak-password': errorMessage = 'Password must be at least 6 characters long.'; break;
               case 'auth/invalid-email': errorMessage = 'Please enter a valid email address.'; break;
               case 'auth/user-not-found': // During login
               case 'auth/wrong-password': // During login (Firebase often uses generic codes now)
               case 'auth/invalid-credential': // Generic login failure
                   errorMessage = 'Invalid email or password.'; break;
               default: errorMessage = err.message; // Use default message
           }
       } else if (err instanceof Error) { // Handle errors from our backend API call
  ...
  // Rest of LoginView JSX remains the same...
  return (
    <div className="flex items-center justify-center min-h-screen w-full bg-gray-950">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md p-8 m-4 bg-gray-900 border border-gray-700/50 rounded-lg shadow-2xl">
         ...
         <form onSubmit={handleAuth}>
             <div className="mb-4">
                 <label className="block text-sm font-medium text-gray-300 mb-2" htmlFor="email">Student ID (Email)</label>
                 <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 bg-gray-800 text-white border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="e.g., 12345@college.edu" required />
             </div>
             <div className="mb-6">
                 <label className="block text-sm font-medium text-gray-300 mb-2" htmlFor="password">Password</label>
                 <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 bg-gray-800 text-white border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="••••••••" required />
             </div>
             ...
             <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 px-5 py-3 text-white font-semibold bg-linear-to-r from-indigo-600 to-purple-600 rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-colors disabled:opacity-50">
         ...
         <p className="text-center text-sm text-gray-400 mt-6">
             ...
             <button onClick={() => { setIsRegistering(!isRegistering); setError(''); }} className="font-medium text-indigo-400 hover:text-indigo-300 ml-1">
...
// --- Other Components (Sidebar, Header, Views...) remain largely the same ---
// Note: Ensure all other components are included below this point as they were before.
// I'll only include the main App component structure for brevity, assuming others are unchanged.

const Sidebar = ({ view, setView, isMobileMenuOpen, setIsMobileMenuOpen }) => { /* ... (No changes needed) ... */
...
const VerificationView = () => { /* ... (No changes needed, still calls /api/getVoteDetails) ... */
...
export default function App() {
  ...
  if (loadingAuth) {
    return ( <div className="flex items-center justify-center h-screen w-full bg-gray-950"><LoadingSpinner /><span className="ml-2 text-white">Loading...</span></div> );
  }
