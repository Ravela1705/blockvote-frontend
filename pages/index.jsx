...
// --- *** UPDATED ELECTION VIEW *** ---
const ElectionView = () => {
    const [election, setElection] = useState(null); // State for fetched election data
    const [candidates, setCandidates] = useState([]);
    const [selectedCandidate, setSelectedCandidate] = useState(null);
    const [loading, setLoading] = useState(true); // Start loading true
    const [voteLoading, setVoteLoading] = useState(false);
    const [message, setMessage] = useState({ type: '', text: '' });
    const [electionStatus, setElectionStatus] = useState('Loading...'); // Loading, Active, Upcoming, Ended
    ...
    const fetchElectionData = async () => {
        ...
        setElection(null); // Clear previous election
        setCandidates([]);

        try {
            // Get read-only provider and contract instance
            ...
            console.log("Total elections created:", electionCount);

            if (electionCount === 0) {
                 setElectionStatus('No elections found.');
                 setLoading(false);
                 return;
            }

            // For simplicity, let's fetch the *latest* election (ID = electionCount)
            const latestElectionId = electionCount;
            console.log("Fetching details for latest election ID:", latestElectionId);

            // Fetch basic details (name, start, end)
            const details = await contract.getElectionDetails(latestElectionId);
            const name = details[0];
            const startTime = details[1].toNumber(); // Convert BigNumber
            const endTime = details[2].toNumber();   // Convert BigNumber
            console.log("Election details:", name, startTime, endTime);
            ...
            const formattedCandidates = candidatesData.map(c => ({
                // Safely convert potential BigNumbers to numbers
                id: ethers.BigNumber.isBigNumber(c.id) ? c.id.toNumber() : parseInt(c.id, 10),
                name: c.name,
                // voteCount not needed for voting view, but could be fetched
            }));
            console.log("Candidates:", formattedCandidates);


            // Check election status based on time
            ...
            console.log("Current time:", now, "Status:", status);
            ...
            setMessage({ type: 'error', text: 'Could not load election data from the blockchain.' });
    ...
    // Vote function (mostly same, just uses state)
    const handleVote = async (electionId, candidateId) => {
        setVoteLoading(true); setMessage({ type: '', text: '' });
        const { data: { session }, error: sessionError } = await supabase.auth.getSession();
        if (sessionError || !session) { setMessage({ type: 'error', text: "Auth error." }); console.error("Session error:", sessionError); setVoteLoading(false); return; }
        ...
        try {
            ...
            setMessage({ type: 'success', text: `Success! Vote recorded. TX Hash: ${data.transactionHash.substring(0, 10)}...` });
            // Optionally disable voting or refresh voter status here
        } catch (err) { let errorMessage = err.message || 'Unknown error.'; setMessage({ type: 'error', text: errorMessage }); }
        setVoteLoading(false);
    };

    // --- Render Logic ---
    if (loading) {
        return <div className="p-8 flex justify-center items-center"><LoadingSpinner /> <span className="ml-2">Loading Election...</span></div>;
    }

    if (!election) {
         return <div className="p-8 text-center text-gray-400">{electionStatus}</div>; // Show status like "No elections found" or "Error"
    }
    ...
    return (
        <div className="p-4 md:p-8">
             <div className="flex justify-between items-center mb-4">
                 ...
                 <button onClick={fetchElectionData} disabled={loading} className="p-2 text-gray-400 hover:text-white disabled:opacity-50">
                    <RefreshCw size={18} className={loading ? 'animate-spin' : ''}/>
                 </button>
            </div>
             <div className={`mb-6 p-3 rounded-lg border text-center ${ electionStatus === 'Active' ? 'bg-green-900/50 border-green-700 text-green-300' : electionStatus === 'Ended' ? 'bg-red-900/50 border-red-700 text-red-300' : 'bg-yellow-900/50 border-yellow-700 text-yellow-300' }`}>
                Status: {electionStatus}
                {electionStatus === 'Active' && election.endTime && (
                    <span className="text-xs ml-2">(Ends: {new Date(election.endTime * 1000).toLocaleString()})</span>
                )}
                 {electionStatus === 'Upcoming' && election.startTime && (
                    <span className="text-xs ml-2">(Starts: {new Date(election.startTime * 1000).toLocaleString()})</span>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                <h3 className="text-xl font-semibold text-white mb-4">Candidates</h3>
                 {candidates.length === 0 && <p className="text-gray-400">Loading candidates...</p>}
                <div className="flex flex-col gap-3">
                    {candidates.map(candidate => (
                    <button
                        key={candidate.id}
                        onClick={() => setSelectedCandidate({ election: election.id, candidate: candidate.id })}
                        disabled={electionStatus !== 'Active'} // Disable if election not active
                        className={`w-full text-left p-4 rounded-lg transition-all ${
                            selectedCandidate?.candidate === candidate.id && electionStatus === 'Active'
                ...
                <button
                    onClick={() => { if(selectedCandidate) { handleVote(selectedCandidate.election, selectedCandidate.candidate) } }}
                    disabled={!selectedCandidate || selectedCandidate.election !== election.id || isVoteDisabled}
                ...
                >
                    {voteLoading ? <LoadingSpinner /> : (message.type === 'success' ? <CheckCircle /> : <Vote size={18} />)}
                    <span>
                        {message.type === 'success' ? 'Voted Successfully' :
                         voteLoading ? 'Submitting...' :
                         electionStatus !== 'Active' ? `Election ${electionStatus}` :
                         !selectedCandidate ? 'Select a Candidate' :
                         'Cast Your Vote'}
                    </span>
                </button>
                </motion.div>
...
// --- Other Views (DashboardHome, ResultsView, VerificationView - SAME AS BEFORE) ---
